...
DEFAULT_VIDEO_POOL = Path("data/autorobobench/robocasa_stand_mixer_peak_video_pool.json")
DEFAULT_VIEWS = ("robot0_agentview_left", "robot0_agentview_right")


def main() -> None:
    parser = argparse.ArgumentParser(description="Verify the RoboCasa stand-mixer peak task assets.")
...
def verify_assets(manifest_path: Path, split_path: Path, video_pool_path: Path, *, verify_files: bool) -> dict:
    ...
    if video_pool.get("target_task_included") is not False:
        raise ValueError(f"{video_pool_path} must exclude the target stand-mixer task")

    manifest_tasks = {task["alias"]: task for task in manifest["tasks"]}
    if set(manifest_tasks) != {"PickPlaceCounterToStandMixer"}:
        raise ValueError("stand-mixer peak manifest must contain only PickPlaceCounterToStandMixer")
    ...
    for task in video_pool["tasks"]:
        ...
        if alias == "PickPlaceCounterToStandMixer":
    ...
    return {
        "task": "robocasa_stand_mixer_peak",
