...
CHANGE = 'Flow matching action policy with wrist dropout on libero_easy5'
...
VIDEO_AUX = False
DREAMZERO = False
DREAMZERO_AUX_WEIGHT = 0.25
DREAMZERO_PSEUDO_SAMPLES = 1000
...
def main() -> None:
    ...
    aux_weight = 0.0
    if DREAMZERO:
        aux_metrics = _run_dreamzero_aux(out_dir, args.max_train_seconds)
        aux_data = str(out_dir / "pseudo" / "pseudo_paired.npz")
        aux_weight = DREAMZERO_AUX_WEIGHT
    elif VIDEO_AUX:
    ...
    metrics["dreamzero"] = DREAMZERO
    metrics["dreamzero_aux_weight"] = aux_weight
...
def _run_dreamzero_aux(out_dir: Path, max_train_seconds: float) -> dict:
    tok_dir = out_dir / "tokenizer"
    world_dir = out_dir / "world_model"
    inverse_dir = out_dir / "inverse"
    pseudo_dir = out_dir / "pseudo"
    tok_budget = max(1.0, 0.15 * max_train_seconds)
    world_budget = max(1.0, 0.15 * max_train_seconds)
    inverse_budget = max(1.0, 0.15 * max_train_seconds)
    _run(
        [
            "python",
            "train/train_tokenizer.py",
    ...
    _run(
        [
            "python",
            "train/train_dreamzero_pseudo.py",
            "--video-data",
            str(prepare.VIDEO_SHARD),
            "--paired-data",
            str(prepare.PAIRED_SHARD),
            "--manifest",
            str(prepare.DATA_DIR / "manifest.json"),
            "--tokenizer",
            str(tok_dir / "tokenizer.pt"),
            "--inverse",
            str(inverse_dir / "inverse.pt"),
            "--out-dir",
            str(pseudo_dir),
            "--max-samples",
            str(DREAMZERO_PSEUDO_SAMPLES),
            "--batch-size",
            str(BATCH_SIZE),
            "--device",
            DEVICE,
    ...
    tok_metrics = json.loads((tok_dir / "metrics.json").read_text())
    world_metrics = json.loads((world_dir / "metrics.json").read_text())
    inverse_metrics = json.loads((inverse_dir / "metrics.json").read_text())
    pseudo_metrics = json.loads((pseudo_dir / "metrics.json").read_text())
    return {
        "video_loss": tok_metrics.get("video_loss"),
        "tokenizer_loss": tok_metrics.get("tokenizer_loss"),
        "val_video_nll": world_metrics.get("val_video_nll"),
        "action_mse": inverse_metrics.get("action_mse"),
        "pseudo_samples": pseudo_metrics.get("pseudo_samples"),
        "pseudo_path": pseudo_metrics.get("pseudo_path"),
    }
